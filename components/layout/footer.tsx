...
export function Footer() {
  ...
  const accountLinks = [
    ...
    { href: "/subscribe", label: isSubscribed ? "Mi suscripción" : "Suscribirse" },
  ]

  return (
    <footer className="relative z-10 mt-24 pb-8 pt-10">
      <div className="container mx-auto px-4 md:px-8">
        <div className="relative overflow-hidden rounded-[34px] border border-border/50 bg-card/95 px-6 py-8 shadow-[0_30px_120px_rgba(0,0,0,0.10)] dark:shadow-[0_30px_120px_rgba(0,0,0,0.28)] md:px-8 md:py-10">
          ...
          <div className="relative grid gap-10 lg:grid-cols-[minmax(0,1.25fr)_0.8fr_0.8fr_0.8fr]">
            <div className="space-y-5">
              <div className="inline-flex items-center gap-2 rounded-full border border-border/40 bg-[var(--glass-bg)] px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.28em] text-muted-foreground">
                <span className="h-1.5 w-1.5 rounded-full bg-primary" />
                Biblioteca Curada
              </div>

              <div className="space-y-3">
                <h2 className="font-display text-4xl font-semibold tracking-tight text-foreground md:text-[3.2rem]">
                  ALLYN
                </h2>
                <p className="max-w-xl text-sm leading-relaxed text-foreground/62 md:text-base">
          ...
          <div className="relative mt-10 flex flex-col gap-4 border-t border-border/30 pt-6 md:flex-row md:items-center md:justify-between">
...
function FooterColumn({
  ...
  return (
    <div>
      <p className="text-[11px] uppercase tracking-[0.28em] text-foreground/38">{title}</p>
